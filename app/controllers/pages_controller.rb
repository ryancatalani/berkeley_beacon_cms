...
class PagesController < ApplicationController
	...
	def search
		...
		if params[:q]
			...
			@articles = results.records['articles'].map{|r| Article.find(r.external_id.to_i) rescue nil } #.paginate(:page => params[:page], :per_page => 15)
			@mediafiles = results.records['mediafiles'].map{|r| Mediafile.find(r.external_id.to_i) rescue nil }
		else
			@articles = []
			@mediafiles = []
		end
