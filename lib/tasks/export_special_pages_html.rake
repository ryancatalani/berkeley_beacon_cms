...
namespace :export do
	...
	task :special_pages_html => :environment do
		...
		projects.each do |route, project|
			...
			route_components = route.split("/").reject(&:blank?)
			fname = route_components.pop
			puts fname
			...
			FileUtils.mkdir_p (Rails.root.join("archives", *route_components))
			File.open(Rails.root.join("archives", *route_components, "#{fname}.html"), "w") do |f|
				f.puts noko.to_s
			end
