...
namespace :export do
	desc "Export special pages to sitemap"
	task :special_pages_sitemap => :environment do

		projects = {
			'/projects/election_guide_2012' => {
				year: 2012,
			...
			'/projects/election_guide_2012/poll_results' => {
				year: 2012,
			...
			'/projects/election_guide_2012/map' => {
				year: 2012,
				action: 'election_map',
				locals: {
					:@og => {
		...
		builder = Nokogiri::XML::Builder.new do |xml|
			xml.urlset("xmlns" => "http://www.sitemaps.org/schemas/sitemap/0.9", "xmlns:news" => "http://www.google.com/schemas/sitemap-news/0.9") do
				...
				projects.each do |route, project|
					...
					full_url = [base_url, route, ".html"].join("")
					date = Date.new(project[:year], 06, 01).strftime("%Y-%m-%d")
					...
					xml.url do
						...
						xml['news'].news do
							...
							xml['news'].publication_date date
							xml['news'].title project[:locals][:@title]
						end
					end
				end
		...
		File.open(Rails.root.join("archives", "sitemap_projects.xml"), "w") do |f|
